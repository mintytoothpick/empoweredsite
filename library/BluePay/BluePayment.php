...
class BluePayment {
    ...
    const ACCOUNT_ID = '100098527383'; // the default account id
    const SECRET_KEY = '7ZL24E7KBYTWLL4DSDHJ.NKB6W7MRAJH'; // the default secret key
    ...
    */
    public function setCustACHInfo($routenum, $accntnum, $accttype, $name1, $name2,
        $addr1, $city, $state, $zip, $country, $phone, $email, $customid1 = null,
        $customid2 = null, $addr2 = null, $memo = null
    ) {
    ...
    */
    public function setCustInfo($account, $cvv2, $expire, $name1, $name2,
        $addr1, $city, $state, $zip, $country, $phone, $email,
        $customid1 = null, $customid2 = null, $addr2 = null, $memo = null
    ) {
    ...
    */
     ...
     * setRebillId()
     *
     * Sets the REBILL_ID parameter.
     */
    public function setRebillId($rebid) {
        $this->rebid = $rebid;
    }
    ...
    * Calculates & returns the tamper proof seal md5.
    */
    protected final function calcTpsReb() {
        $hashstr = $this->secretKey . $this->accountId . $this->transType . $this->rebid;
    ...
    */
    ...
    public function stopRebill() {
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://secure.bluepay.com/interfaces/bp20rebadmin'); // Set the URL
        curl_setopt($ch, CURLOPT_USERAGENT, "BluepayPHP SDK/2.0"); // Cosmetic
        curl_setopt($ch, CURLOPT_POST, 1); // Perform a POST
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0); // Turns off verification of the SSL certificate.
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); // If not set, curl prints output to the browser
        ...
        if (strpos($this->response, 'status=stopped') > 0) {
            $this->status = 'stopped';
        }
    }
