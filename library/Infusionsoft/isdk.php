...
public function setCon($uri, $key, $ssl, $dbOn="on") {
  $this->debug = $dbOn;
  ...
  $this->client->setSSLVerifyPeer($ssl);
...
###public function for ContactService.add method###
public function csAdd($iMap) {
    $carray = array(
                    php_xmlrpc_encode($this->key),
                    php_xmlrpc_encode($iMap,array('auto_dates')));

    return $this->methodCaller("ContactService.add",$carray);
}

###public function for ContactService.update method###
public function csUpdate($id,$iMap) {
